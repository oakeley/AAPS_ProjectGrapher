...
class EnhancedNeo4jRAGRetriever:
    ...
    def search_eating_now_source_code(self, keywords: List[str], repository: str = None, limit: int = 12) -> List[Dict]:
        ...
        # If fulltext search doesn't yield enough results, fall back to property search
        if len(fulltext_results) < limit // 2:
            ...
            seen_files = {r['filename'] for r in fulltext_results}
    ...
    def get_eating_now_function_examples(self, function_type: str) -> List[Dict]:
        """Get specific eating now function examples with source code"""
        function_types = {
            'bolus': ['bolus', 'dose', 'insulin'],
            'carb': ['carb', 'carbohydrate', 'nutrition'],
            'calculation': ['calc', 'compute', 'algorithm'],
            'timing': ['time', 'schedule', 'delay'],
            'validation': ['valid', 'check', 'verify']
        }
        
        terms = function_types.get(function_type.lower(), [function_type])
        
        query = """
        MATCH (fn:Function)
        WHERE fn.eating_now_related = true
        AND ANY(term IN $terms WHERE toLower(fn.name) CONTAINS term)
        MATCH (f:File {name: fn.file_name, repository: fn.repository})
        RETURN fn.name as function_name,
               fn.source_code as function_source,
               f.name as filename,
               f.repository as repository,
               f.eating_now_score as file_eating_score,
               f.package as package
        ORDER BY f.eating_now_score DESC
        LIMIT 10
        """
...
class EnhancedOllamaClient:
    ...
    def generate_eating_now_response(self, prompt: str, context: str = "", cached_code_summary: str = "", max_tokens: int = 3072) -> str:
        ...
        max_context_length = 12000  # Increased for source code
...
class EnhancedAAPSRAGSystem:
    ...
    def retrieve_eating_now_context(self, question: str) -> str:
        ...
        # Get function examples for specific functionality
        function_keywords = ['bolus', 'carb', 'calculation', 'timing', 'validation']
        for func_keyword in function_keywords:
            if func_keyword in keywords:
                examples = self.retriever.get_eating_now_function_examples(func_keyword)
                if examples:
                    self._add_function_examples_context(context_parts, func_keyword, examples)
    ...
    def _add_eating_now_overview_context(self, context_parts: List[str], overview: Dict, target_repo: str):
        ...
        if overview.get('top_eating_now_files'):
            ...
            for file_info in overview['top_eating_now_files'][:6]:
                ...
                context_parts.append(f"   • {file_info['filename']} ({repo}): Score {score:.1f}")
                # Add source preview
        ...
        if overview.get('eating_now_repositories'):
            ...
            for repo_info in overview['eating_now_repositories']:
                context_parts.append(f"   • {repo_info['repository']}: Avg Score {repo_info['avg_eating_now']:.1f}, {repo_info['files']} files")
    ...
    def _add_source_code_context(self, context_parts: List[str], relevant_files: List[Dict], include_full_source: bool = False):
        ...
        for file_info in relevant_files[:5]:  # Top 5 files
            ...
            context_parts.append(f"\n📄 {filename} ({repo}) - Eating Score: {score:.1f}")
    ...
    def _add_template_context(self, context_parts: List[str], keyword: str, templates: List[Dict]):
        ...
        for template in templates[:3]:  # Top 3 templates
            ...
            context_parts.append(f"\n   📄 {template['filename']} ({repo}) - Score: {score:.1f}")
    ...
    def _add_function_examples_context(self, context_parts: List[str], func_keyword: str, examples: List[Dict]):
        """Add function examples with actual source code"""
        context_parts.append(f"\n🔍 {func_keyword.upper()} FUNCTION EXAMPLES:")
        
        for example in examples[:3]:  # Top 3 examples
            func_name = example.get('function_name', 'unknown')
            filename = example.get('filename', 'unknown')
            repo = example.get('repository', 'unknown')
            
            context_parts.append(f"\n   🔧 {func_name}() in {filename} ({repo})")
            
            func_source = example.get('function_source', '')
            if func_source:
                # Show function signature and first few lines
                lines = func_source.split('\n')
                context_parts.append("      Function Code:")
                for line in lines[:8]:  # First 8 lines
                    if line.strip():
                        context_parts.append(f"        {line}")
                if len(lines) > 8:
                    context_parts.append("        [... function continues ...]")
    ...
    def show_eating_now_stats(self):
        ...
        if overview.get('top_eating_now_files'):
            ...
            for i, file_info in enumerate(overview['top_eating_now_files'][:8], 1):
                repo = file_info.get('repository', 'unknown')
                score = file_info.get('eating_now_score', 0)
                print(f"   {i:2d}. {file_info['filename']} ({repo}): {score:.1f}")
        ...
        for template_type in template_types:
            ...
            if templates:
                ...
                for template in templates[:3]:
                    repo = template.get('repository', 'unknown')
                    score = template.get('eating_now_score', 0)
                    print(f"   • {template['filename']} ({repo}): Score {score:.1f}")
